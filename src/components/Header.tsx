  ...
  return (
    <header className="border-b border-white/[0.08] bg-bg-card px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <div className="flex flex-col gap-2.5">
        {/* Row 1: Focus + Server status — compact horizontal strip */}
        <div className="flex items-center gap-2 text-xs overflow-x-auto no-scrollbar">
          {botState !== "STATE_IDLE" && (
            <div className="flex items-center gap-1.5 bg-[#1c1c22] px-2.5 py-1.5 rounded-lg border border-white/[0.06] shrink-0">
              <span className="text-neutral-500 uppercase tracking-wide text-[8.5px] font-bold">Focus:</span>
              <span className="font-mono text-white flex items-center gap-1.5 text-[10px] font-semibold whitespace-nowrap">
          ...
          <div className="flex items-center gap-1.5 bg-[#1c1c22] px-2.5 py-1.5 rounded-lg border border-white/[0.06] shrink-0">
            <span className="text-neutral-500 uppercase tracking-wide text-[8.5px] font-bold">Server:</span>
            <span className="flex items-center gap-1.5 font-semibold text-[10px] whitespace-nowrap">
              <span
        ...
        {/* Row 2: Balance card + Telegram badge — compact horizontal strip */}
        <div className="flex items-center gap-2 flex-wrap">
          {balance !== null && (
            <div className="flex items-center gap-2.5 bg-[#1c1c22] pl-3 pr-3.5 py-2 rounded-lg border border-white/[0.06] text-xs shadow-md">
              <div className="flex items-center gap-1.5 font-bold">
                {isRealAccount ? (
                  <ShieldCheck className="h-3.5 w-3.5 text-gold-500" />
                ) : (
                  <User className="h-3.5 w-3.5 text-orange-400 animate-pulse font-bold" />
                )}
                <span
                  className={`font-bold tracking-wider text-[8px] uppercase px-1.5 py-0.5 rounded ${
              ...
              <div className="flex flex-col min-w-[64px]">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[8px] text-neutral-500 leading-3 uppercase font-bold tracking-wider">Balance</span>
                  {!isRealAccount && resetDemoBalance && (
                    <button
                      type="button"
                      onClick={resetDemoBalance}
                      className="text-[7.5px] text-orange-400 hover:text-orange-300 uppercase tracking-widest font-black leading-3 cursor-pointer underline hover:no-underline transition-all"
          ...
          {telegramUser && (
            <div className="flex items-center gap-1.5 bg-[#229ED9]/10 border border-[#229ED9]/20 pl-2.5 pr-3 py-1.5 rounded-lg text-xs animate-fade-in shadow-sm">
              <span className="h-1.5 w-1.5 rounded-full bg-[#229ED9] animate-pulse shrink-0" />
              <span className="text-[10px] text-neutral-300 font-sans font-medium flex items-center gap-1 whitespace-nowrap">
                <span className="font-bold text-[#229ED9]">@{telegramUser.username || telegramUser.first_name}</span>
              </span>
            </div>
          )}
        </div>

        {/* Row 3: Login status — only shown when relevant */}
        {currentUserEmail && (
          <div className="flex items-center gap-2.5 bg-rose-500/5 hover:bg-rose-500/10 border border-rose-500/15 pl-3 pr-2 py-1.5 rounded-lg text-xs transition-colors duration-200 w-fit">
